...
(()=>{
  ...
  try{
    if(typeof EasyInstall!=='undefined'){
      ...
      EasyInstall.open=()=>{
        if(isStandalone()){
          try{localStorage.setItem('oneshotInstallDismissed','1');}catch(_){}
          closeInstallModal();
          EasyInstall.render?.();
          if(typeof UI!=='undefined')UI.toast('ONE SHOT ya está instalada en este dispositivo',2600);
          return;
        }
        return originalOpen();
      };
    ...
    if(typeof AppUpdater!=='undefined'){
      ...
      AppUpdater.check=async(force=false)=>{
        ...
        try{
          ...
          if(same){
            markCurrent();closeUpdateModal();
            if(txt)txt.textContent=`Actualizado · ${remote?.version||'v6.2'}`;
            if(force&&typeof UI!=='undefined')UI.toast(`ONE SHOT ${remote?.version||'v6.2'} ya está actualizado`,2500);
            return;
          }
          ...
          localStorage.setItem('oneshotAppliedBuild',remoteBuild);
      ...
      setTimeout(async()=>{
        try{const remote=await AppUpdater.feed();if(!remote?.build||remote.build===BUILD||remote.build===localStorage.getItem('oneshotAppliedBuild')){markCurrent();closeUpdateModal();}}
        catch(_){}
      },0);
    }
  }catch(e){console.warn('ONE SHOT update controller',e);}
