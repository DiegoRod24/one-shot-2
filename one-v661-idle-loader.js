/* ONE SHOT v6.6.17 · CAMERA-FIRST IDLE LOADER */
(()=>{
'use strict';
if(window.ONE_V661_IDLE_LOADER)return;
const BUILD='oneshot-v6.6.17-camera-first-idle-loader-01';
const FEATURES={
  ...
  territory:['one-v6413-corridor.js','one-v6617-tramo-postes.js','one-v6413-corridor-reports.js','one-v6617-tramo-reports.js','one-v6415-territory-ops.js','one-v653-field-findings.js']
};
...
async function ensure(name){for(const src of FEATURES[name]||[])await loadScript(src);if(name==='territory'){try{window.ONE_V6413_CORRIDOR?.start?.()}catch(_){}try{window.ONE_V6617_TRAMO_POSTES?.start?.()}catch(_){}try{window.PropagandaCorridor?.injectV6617?.();window.PropagandaCorridor?.paint?.()}catch(_){}try{window.ONE_V6413_CORRIDOR_REPORTS?.start?.();window.ONE_V6617_TRAMO_REPORTS?.start?.()}catch(_){}}return true}
...
function schedule(){
  // La cámara mantiene prioridad absoluta; módulos pesados esperan video activo o intención explícita.
