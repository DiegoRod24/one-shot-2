/* ONE SHOT v6.6.28 · CAMERA-FIRST LAZY RUNTIME */
(()=>{
'use strict';
if(window.ONE_V661_IDLE_LOADER)return;
const BUILD='oneshot-v6.6.28-camera-first-lazy-runtime-01';
const FEATURES={
  mobile:['one-v660-political-activity.js','one-v664-editor-stable.js','one-v6615-nearby-preferences.js','one-v6618-field-flow.js','one-v667-mobile-batch.js','one-v668-mobile-editor-polish.js','one-v669-local-partidario.js'],
  cloud:['one-v6616-sync-identity.js','one-sync-worker-mode.js','one-dropbox-sync.js','one-phase2-edit-center.js'],
...
function afterCameraReady(fn,maxWait=12000){const started=performance.now();const tick=()=>{if(cameraLive()){setTimeout(fn,120);return}if(performance.now()-started>=maxWait)return;setTimeout(tick,180)};tick()}
function idle(fn,timeout){if('requestIdleCallback' in window)requestIdleCallback(fn,{timeout});else setTimeout(fn,Math.min(timeout,4500))}
function schedule(){
  // Solo UI de evidencia de uso frecuente y recién DESPUÉS de que la cámara esté viva.
  // Nube, mapas, municipio, reportes y territorio cargan exclusivamente por intención.
  afterCameraReady(()=>idle(()=>ensure('mobile').catch(e=>console.warn('[ONE SHOT PERF] mobile',e)),2600),12000);
}
...
document.addEventListener('pointerdown',warmByIntent,{capture:true,passive:true});window.addEventListener('load',start,{once:true});if(document.readyState==='complete')start();window.ONE_V661_IDLE_LOADER={BUILD,ensure,intent,FEATURES};
})();
