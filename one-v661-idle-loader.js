/* ONE SHOT v6.6.20 · CAMERA-FIRST IDLE LOADER */
(()=>{
'use strict';
if(window.ONE_V661_IDLE_LOADER)return;
const BUILD='oneshot-v6.6.20-camera-first-idle-loader-01';
const FEATURES={
...
async function ensure(name){if(name==='territory')await ensure('tramo');for(const src of FEATURES[name]||[])await loadScript(src);if(name==='tramo'||name==='territory'){try{window.ONE_V6413_CORRIDOR?.start?.()}catch(_){}try{window.ONE_V6617_TRAMO_POSTES?.start?.()}catch(_){}try{const c=typeof PropagandaCorridor!=='undefined'?PropagandaCorridor:window.PropagandaCorridor;c?.injectV6617?.();c?.paint?.()}catch(_){}try{window.ONE_V6620_TRAMO_MAP?.prepare?.()}catch(_){}try{window.ONE_V6413_CORRIDOR_REPORTS?.start?.();window.ONE_V6617_TRAMO_REPORTS?.start?.()}catch(_){}}return true}
...
function schedule(){afterCameraReady(()=>idle(()=>ensure('mobile').catch(e=>console.warn('[ONE SHOT PERF] mobile',e)),1800),12000);afterCameraReady(()=>idle(()=>ensure('cloud').catch(e=>console.warn('[ONE SHOT PERF] cloud',e)),3600),15000);afterCameraReady(()=>idle(()=>ensureAdmin().catch(e=>console.warn('[ONE SHOT PERF] admin',e)),7500),18000)}
