const CACHE="oneshot-v6.6.1-performance-runtime-slim-01";
const RUNTIME_BUNDLE="./__oneshot_runtime_v661.js";
...
const DYNAMIC=[
  ...
  "one-v651-assets.js","one-v651-domain.js","one-v651-municipal.js","one-v651-edit-core.js","one-v651-fer-shell.js","one-v653-mobile-ux.js"
];
...
const FER_ASSETS=["assets/fer/fer-pose-01.webp","assets/fer/fer-pose-02.webp","assets/fer/fer-pose-03.webp","assets/fer/fer-pose-04.webp","assets/fer/fer-pose-05.webp","assets/fer/fer-pose-06.webp"];
const ASSETS=["./","index.html","styles.css","app.js","version.json","manifest.json","oneshot-erm-data.js",...DYNAMIC,"one-v661-idle-loader.js",...FER_ASSETS,"oneshot-logo.svg","oneshot-mark.png","oneshot-mark-transparent.png","icon-192.png","icon-512.png"];
...
self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;const u=new URL(e.request.url);if(u.origin!==location.origin)return;const isVersion=u.pathname.endsWith("version.json"),isData=u.pathname.endsWith("oneshot-erm-data.js"),isFieldAsset=u.pathname.includes("/assets/parties/")||u.pathname.includes("/assets/fer/"),isLazy=LAZY.some(x=>u.pathname.endsWith(`/${x}`)||u.pathname.endsWith(x));if(isData){e.respondWith((async()=>{const c=await caches.open(CACHE),dataPromise=fetch(e.request,{cache:"no-store"}).catch(()=>c.match(e.request)),bundlePromise=runtimeBundle(c),[data,bundle]=await Promise.all([dataPromise,bundlePromise]);if(!data)throw new Error("oneshot-erm-data.js no disponible");return new Response(`${await data.text()}\n${await bundle.text()}`,{headers:{"Content-Type":"application/javascript; charset=utf-8","Cache-Control":"no-store"}})})());return}if(isFieldAsset||isLazy){e.respondWith((async()=>{const c=await caches.open(CACHE),hit=await c.match(e.request);if(hit)return hit;const r=await fetch(e.request,{cache:"reload"});if(r.ok)await c.put(e.request,r.clone());return r})());return}e.respondWith((isVersion?fetch(e.request,{cache:"no-store"}):fetch(e.request)).then(r=>{if(!isVersion&&r.ok){const copy=r.clone();caches.open(CACHE).then(c=>c.put(e.request,copy)).catch(()=>{})}return r}).catch(()=>caches.match(e.request)))});
