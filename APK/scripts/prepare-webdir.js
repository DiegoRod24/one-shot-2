...
function arrayConst(name){const m=sw.match(new RegExp(`const\\s+${name}\\s*=\\s*(\\[[\\s\\S]*?\\]);`));if(!m)throw new Error(`[ONE SHOT] No pude leer ${name} de service-worker.js`);const a=JSON.parse(m[1]);if(!Array.isArray(a))throw new Error(`[ONE SHOT] ${name} inválido`);return a;}
...
console.log(`[ONE SHOT] WebDir listo en www/ · ${files.length} archivos sincronizados · ${dynamicFiles().length} core + ${lazyFiles().length} lazy`);
