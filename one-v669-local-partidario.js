...
#oneV669QuickLocal{border:1px solid rgba(25,177,125,.45);background:rgba(14,117,82,.16);color:#0b684b;border-radius:14px;padding:9px 12px;font-weight:900;white-space:nowrap}
#quickCaptureBar #oneV669QuickLocal{min-height:38px;padding:7px 9px;font-size:12px;color:#fff;background:rgba(13,119,83,.92);border-color:rgba(255,255,255,.2)}
#oneV664QuickEditor #oneV669QuickLocal{width:100%;margin-top:10px;color:#dffcf2;background:rgba(15,130,90,.22);border-color:rgba(55,211,151,.35);font-size:14px}
...
@media(max-width:380px){#quickCaptureBar #oneV669QuickLocal{font-size:0;width:38px;padding:5px}#quickCaptureBar #oneV669QuickLocal::after{content:'🏢';font-size:16px}}
`;
...
function refreshButtons(){
  const bar=q('#quickCaptureBar');if(bar&&!q('#oneV669QuickLocal',bar)){const b=document.createElement('button');b.id='oneV669QuickLocal';b.type='button';b.textContent='🏢 Local';const edit=q('#quickCaptureEdit',bar),close=q('#quickCaptureClose',bar);(edit||close)?.insertAdjacentElement('afterend',b);b.onclick=()=>LocalFlow.open()}
  const quick=q('#oneV664QuickEditor');if(quick&&!q('#oneV669QuickLocal',quick)){const b=document.createElement('button');b.id='oneV669QuickLocal';b.type='button';const r=record();b.textContent=isLocal(r)?'🏢 Local partidario ✓':'🏢 Registrar como local partidario';const st=q('#oneV664EditStatus',quick);if(st)quick.insertBefore(b,st);else quick.appendChild(b);b.onclick=()=>LocalFlow.open(r?.id||'')}
  const r=record();const eb=q('#oneV664QuickEditor #oneV669QuickLocal');if(eb&&r)eb.textContent=isLocal(r)?'🏢 Local partidario ✓':'🏢 Registrar como local partidario';
}
