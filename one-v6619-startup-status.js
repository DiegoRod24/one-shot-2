/* ONE SHOT v6.6.27 · STARTUP STATUS NO BLOQUEANTE + FAST TRAMO */
(()=>{
'use strict';
if(window.ONE_V6619_STARTUP_STATUS)return;
const BUILD='oneshot-v6.6.27-startup-gps-ready-01';
...
#one6619Startup{position:absolute;left:50%;top:max(112px,calc(env(safe-area-inset-top) + 92px));transform:translateX(-50%);z-index:920;width:min(92%,430px);pointer-events:none;transition:opacity .22s,transform .22s;display:none}
...
function addressReady(){const t=String($('wmAddr')?.textContent||'').trim();return !!t&&!/pendiente|buscando|detectad|resolviendo|afinando|completando|gps/i.test(t)}
function cameraReady(){const st=getState();return !!(st?.cameraStatus==='active'&&st.currentTrack?.readyState==='live')}
function hideStartup(host){if(startupDone)return;startupDone=true;host.classList.add('done');setTimeout(()=>host.classList.remove('show'),240);if(timer){clearInterval(timer);timer=null}}
function paint(){
  ...
  if(!camera){title=`Preparando cámara… ${elapsed}s`;sub=elapsed>=6?'La cámara sigue iniciando; GPS no la bloqueará.':'Cámara trasera primero · GPS en paralelo';pct=Math.min(42,12+elapsed*5)}
  else if(!gps){title=`Cámara lista · buscando GPS… ${elapsed}s`;sub='Ya puedes encuadrar. Esperando coordenadas para georreferenciar.';pct=62}
  else{
    title=`✓ Cámara + GPS listos · ±${Math.round(Number(st.gps.accuracy||0))} m`;
    sub=addr?'Dirección lista · puedes capturar.':'Puedes capturar ya · dirección/ubigeo se completa en segundo plano.';
    pct=100;done=true;
  }
  ...
  if(done){if(!readySince)readySince=Date.now();if(Date.now()-readySince>850)hideStartup(host)}else readySince=0;
}
...
async function fastTramo(){
  ...
  }catch(err){console.error('[ONE SHOT v6.6.27 tramo]',err);ui?.toast?.(`No pude abrir Tramo · ${err.message||err}`,3800)}
  finally{unbusy()}
}
...
function boot(){ensure();interceptTramo();paint();timer=setInterval(paint,500)}
