/* ONE SHOT v6.6.28 · STARTUP CÁMARA NO BLOQUEANTE + FAST TRAMO */
(()=>{
'use strict';
if(window.ONE_V6619_STARTUP_STATUS)return;
const BUILD='oneshot-v6.6.28-camera-ready-no-wait-gps-01';
...
let lastText='',readySince=0,busyDepth=0,startupDone=false,timer=null;

function css(){
  if($('one6619StartupCss'))return;
  const s=document.createElement('style');s.id='one6619StartupCss';s.textContent=`
#one6619Startup{position:absolute;left:50%;top:max(112px,calc(env(safe-area-inset-top) + 92px));transform:translateX(-50%);z-index:920;width:min(92%,430px);pointer-events:none;transition:opacity .2s,transform .2s;display:none}
#one6619Startup.show{display:block}#one6619Startup.done{opacity:0;transform:translate(-50%,-7px)}
.one6619StartupCard{display:flex;align-items:center;gap:10px;padding:9px 12px;border:1px solid rgba(255,255,255,.25);border-radius:16px;background:rgba(4,14,34,.84);backdrop-filter:blur(10px);color:#fff;box-shadow:0 8px 24px rgba(0,0,0,.18)}
.one6619Spin{width:20px;height:20px;border:3px solid rgba(255,255,255,.25);border-top-color:#fff;border-radius:50%;animation:one6619spin .8s linear infinite;flex:0 0 auto}.one6619Spin.ok{animation:none;border:0;width:22px;height:22px;display:grid;place-items:center;background:#17865f}.one6619Spin.ok:after{content:'✓';font-weight:950}.one6619StartupText{min-width:0;display:grid;gap:1px}.one6619StartupText b{font-size:12px;line-height:1.2}.one6619StartupText small{font-size:10px;color:#c7d4e7;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.one6619Progress{height:3px;background:rgba(255,255,255,.18);border-radius:99px;overflow:hidden;margin-top:4px}.one6619Progress i{display:block;height:100%;background:#55a0ff;width:14%;transition:width .25s}
...
function ensure(){
  css();
  if(!$('one6619Startup')){const d=document.createElement('div');d.id='one6619Startup';d.innerHTML='<div class="one6619StartupCard"><span id="one6619StartupSpin" class="one6619Spin"></span><div class="one6619StartupText"><b id="one6619StartupTitle">Preparando cámara…</b><small id="one6619StartupSub">GPS seguirá en paralelo</small><div class="one6619Progress"><i id="one6619StartupBar"></i></div></div></div>';($('cameraStage')||document.body).appendChild(d)}
  if(!$('one6619Busy')){const b=document.createElement('div');b.id='one6619Busy';b.innerHTML='<div class="one6619BusyCard"><span class="one6619Spin"></span><b id="one6619BusyTitle">Preparando…</b><small id="one6619BusySub">ONE SHOT está cargando solo lo necesario.</small></div>';document.body.appendChild(b)}
}
...
function hideStartup(host){if(startupDone)return;startupDone=true;host.classList.add('done');setTimeout(()=>host.classList.remove('show'),220);if(timer){clearInterval(timer);timer=null}}
function paint(){
  ensure();const host=$('one6619Startup'),st=getState();if(!host||startupDone)return;
  const camera=cameraReady(),gps=validGps(st?.gps),elapsed=Math.max(0,Math.floor((performance.now()-started)/1000));
  let title='',sub='',pct=14,done=false;
  if(!camera){title=`Preparando cámara… ${elapsed}s`;sub=elapsed>=5?'La cámara sigue iniciando · puedes tocar ↻ si no responde.':'Abrimos la cámara primero; GPS no bloquea el arranque.';pct=Math.min(80,14+elapsed*10)}
  else{
    title=gps?`✓ Cámara lista · GPS ±${Math.round(Number(st.gps.accuracy||0))} m`:'✓ Cámara lista · GPS en segundo plano';
    sub=gps?'Puedes capturar. Dirección/ubigeo se completa detrás.':'Ya puedes encuadrar; el chip GPS se actualizará cuando lleguen coordenadas.';
    pct=100;done=true;
  }
  ...
  host.classList.add('show');$('one6619StartupSpin').classList.toggle('ok',done);
  if(done){if(!readySince)readySince=Date.now();if(Date.now()-readySince>650)hideStartup(host)}
  // Nunca dejamos un indicador de arranque eterno. La salud de cámara/GPS ya vive en los chips.
  if(elapsed>=10&&camera)hideStartup(host);
}
function busy(title,sub=''){ensure();busyDepth++;$('one6619BusyTitle').textContent=title||'Preparando…';$('one6619BusySub').textContent=sub||'ONE SHOT está cargando solo lo necesario.';$('one6619Busy').classList.add('open')}
...
async function fastTramo(){
  ...
  }catch(err){console.error('[ONE SHOT v6.6.28 tramo]',err);ui?.toast?.(`No pude abrir Tramo · ${err.message||err}`,3800)}
  finally{unbusy()}
}
...
function boot(){ensure();interceptTramo();paint();timer=setInterval(paint,350)}
