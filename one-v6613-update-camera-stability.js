/* ONE SHOT v6.6.28 · ACTUALIZACIÓN ATÓMICA + ENTRADA LIMPIA */
(()=>{
'use strict';
if(window.ONE_V6613_UPDATE_CAMERA_STABILITY)return;
const BUILD='oneshot-v6.6.28-update-enter-clean-01';
...
function currentBuild(){const vals=[window.ONE_SHOT_BOOT?.CURRENT_BUILD,localStorage.getItem('oneshotRuntimeBuild'),localStorage.getItem('oneshotAppliedBuild')].filter(Boolean);return vals.reduce((best,v)=>!best||cmp(v,best)>0?v:best,BUILD)||BUILD}
...
function showLatest(remote){const label=versionLabel(remote);setToolText(`Ya tienes la última versión · ${label}`);progress(100,'ONE SHOT está actualizado',`${label} ya está instalada.`,true);const enter=D('oneV6613EnterApp');if(enter)enter.hidden=true;hideOverlay(1400);toast(`✓ ${label} · última versión instalada`,2200)}
...
async function stopCameraSafely(){try{if(typeof State!=='undefined')State.cameraWanted=false}catch(_){}try{if(typeof Camera!=='undefined'&&Camera.stop)await Promise.race([Camera.stop({keepWanted:false}),sleep(1200)])}catch(_){}try{const v=D('video');if(v?.srcObject){v.srcObject.getTracks?.().forEach(t=>t.stop());v.srcObject=null}}catch(_){}}
async function saveBeforeReload(){try{if(typeof Store!=='undefined'&&Store.saveAll)await Promise.race([Store.saveAll(),sleep(1600)])}catch(_){} }
async function applyUpdate(remote){
  ...
  try{
    ...
    progress(94,'Actualizando ONE SHOT','Abriendo la versión instalada…');await sleep(300);const u=new URL(location.href);u.searchParams.set('refresh',String(Date.now()));u.searchParams.set('v',remote.build||BUILD);location.replace(u.pathname+u.search+u.hash);
  }catch(err){updating=false;progress(100,'No se pudo completar',err?.message||String(err),true);const enter=D('oneV6613EnterApp');if(enter)enter.hidden=true;setToolText('No se pudo actualizar · toca para reintentar');toast(err?.message||String(err),4000);hideOverlay(2600)}
}
...
function enterClean(){
  const b=D('oneV6613EnterApp');if(b){b.disabled=true;b.textContent='Entrando…'}
  try{sessionStorage.setItem('oneshotEnteredAfterUpdate','1')}catch(_){}
  const u=new URL(location.origin+location.pathname);u.searchParams.set('start',String(Date.now()));location.replace(u.pathname+u.search+u.hash);
}
function afterUpdateMessage(){
  let raw='';try{raw=sessionStorage.getItem('oneshotUpdateJustApplied')||''}catch(_){}if(!raw)return;
  try{sessionStorage.removeItem('oneshotUpdateJustApplied')}catch(_){}let info={};try{info=JSON.parse(raw)}catch(_){}
  progress(100,'Actualización completada',`${info.version||info.build||'ONE SHOT'} instalada. Toca “Entrar a ONE SHOT” para iniciar una pantalla limpia.`,true);
  setToolText(`Última versión instalada · ${info.version||info.build||''}`);
  const enter=D('oneV6613EnterApp');if(enter){enter.hidden=false;enter.disabled=false;enter.textContent='Entrar a ONE SHOT'}
}
async function cameraWatchdog(){if(watchdogRan)return;watchdogRan=true;await sleep(3800);let needs=false;try{needs=typeof State!=='undefined'&&State.cameraWanted!==false&&(!State.currentTrack||State.currentTrack.readyState!=='live'||State.cameraStatus!=='active')}catch(_){}if(!needs||document.hidden)return;try{toast('Cámara tardando · recuperando automáticamente…',1800);if(typeof State!=='undefined')State.__oneRearStartupDone=false;await Promise.race([Camera?.stop?.({keepWanted:true}),sleep(700)]);await Promise.race([Camera?.start?.({silent:true,force:true}),sleep(5200)])}catch(_){}await sleep(500);let ok=false;try{ok=State.cameraStatus==='active'&&State.currentTrack?.readyState==='live'}catch(_){}if(!ok)toast('La cámara no respondió. Toca ↻ para reintentar.',3200)}
function boot(){ensureOverlay().hidden=true;document.addEventListener('click',intercept,true);window.addEventListener('load',()=>{setTimeout(afterUpdateMessage,120);setTimeout(cameraWatchdog,220)},{once:true});if(document.readyState==='complete'){setTimeout(afterUpdateMessage,120);setTimeout(cameraWatchdog,220)}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.ONE_V6613_UPDATE_CAMERA_STABILITY={BUILD,checkUpdate,applyUpdate,cameraWatchdog,currentBuild,enterClean};
})();
