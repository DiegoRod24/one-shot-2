...
function patch(){if(typeof Reports==='undefined'||!Reports.makeExcel||Reports.__v6411ConfirmedMunicipal)return;Reports.__v6411ConfirmedMunicipal=true;const previous=Reports.makeExcel.bind(Reports);Reports.makeExcel=async(...args)=>{const data=(Evidence.selectedForReport?.()||[]).slice(),backups=[];for(const r of data){if(confirmed(r))continue;const b={r,values:{}};for(const f of FIELDS){b.values[f]=r[f];r[f]='';}backups.push(b);}try{return await previous(...args);}finally{for(const b of backups)for(const f of FIELDS)b.r[f]=b.values[f];}};}
