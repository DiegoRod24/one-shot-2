...
let pendingTramoEvidenceId='';
let activityDbPromise=null;
...
async function geoSnapshot(){
  const g=validGps(window.State?.gps)?{...State.gps}:null;
  let loc={address:'',district:'',province:'',department:'',ubigeo:'UBIGEO pendiente'};
  if(g&&window.GPS?.reverse){try{loc=await Promise.race([GPS.reverse(g),new Promise((_,rej)=>setTimeout(()=>rej(new Error('geo timeout')),3500))])||loc}catch(_){}}
...
function routeContext(){
  const st=window.State?.settings||{},a=(st.teamAssignments||[]).find(x=>x.id===st.activeTeamAssignmentId)||{};
...
async function saveActivityLocal(){
  ...
  try{
    const d=now(),geo=await geoSnapshot(),who=fieldIdentity(),bytes=new Uint8Array(await file.arrayBuffer());
    const duration=Number($('v660ActivityPreview')?.querySelector('video')?.duration||0)||0;
    const row={id:uuid(),activityCode:code(),entity:'political_activity',schemaVersion:'PoliticalActivityV1',sourceApp:'one-shot-2',sourceVersion:BUILD,createdAt:d.toISOString(),fecha:window.Dates?.date?Dates.date(d):d.toISOString().slice(0,10),hora:window.Dates?.time?Dates.time(d):d.toTimeString().slice(0,8),party,activityType,note,mediaKind:file.type.startsWith('video/')?'VIDEO':'PHOTO',mediaType:file.type||'application/octet-stream',mediaSize:file.size||bytes.byteLength,durationSeconds:duration,gps:geo.gps||null,gpsStatus:geo.gps?'GPS_CAPTURA':'SIN_GPS',accuracy:geo.gps?.accuracy??'',address:geo.address||'',addressStructured:geo.addressStructured||'',department:geo.department||'',province:geo.province||'',district:geo.district||'',ubigeo:geo.ubigeo||'UBIGEO pendiente',googleMapsUrl:geo.gps?`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${geo.gps.latitude},${geo.gps.longitude}`)}`:'',electionProcess:window.State?.settings?.activeProcess||'ERM',reviewStatus:'PENDIENTE',reviewSource:'FIELD_FIRST_FILTER',teamId:who.teamId||'',reviewer:who.reviewer||window.State?.settings?.reviewer||'',deviceId:who.deviceId||'',...routeContext(),syncStatus:'PENDING',syncError:'',cloud:null,updatedAt:d.toISOString(),mediaBlob:bytes};
    ...
    const meta=$('v660ActivityFileMeta');if(meta)meta.textContent=`Guardada · ${row.activityCode}`;
    if($('v660ActivityNote'))$('v660ActivityNote').value='';
...
function patchActivity(){
  const btn=$('v660ActivitySave');if(!btn||btn.dataset.one6618Bound==='1')return false;
  btn.dataset.one6618Bound='1';btn.onclick=null;btn.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();saveActivityLocal()},true);setActivityStatus('Primero se guarda localmente; la nube nunca bloquea el guardado.');return true;
}
...
function decorateEvidence(){
  document.querySelectorAll('#evidenceList .eCard').forEach(card=>{const actions=card.querySelector('.eActions');if(!actions||actions.querySelector('[data-one-local]'))return;const id=card.dataset.id||'',r=window.State?.records?.find?.(x=>String(x.id)===String(id)),b=document.createElement('button');b.type='button';b.dataset.oneLocal=id;b.textContent=r?.entityType==='POLITICAL_LOCATION'?'🏢 Local ✓':'🏢 Local';b.title='Registrar esta misma evidencia como local partidario';actions.insertBefore(b,actions.querySelector('[data-act="delete"]')||null)});
}
function patchGallery(){
  if(!window.Gallery?.render||Gallery.__one6618Local)return;Gallery.__one6618Local=true;const old=Gallery.render.bind(Gallery);Gallery.render=(...a)=>{const out=old(...a);setTimeout(decorateEvidence,0);return out};decorateEvidence();
  const list=$('evidenceList');if(list&&!list.dataset.one6618Local){list.dataset.one6618Local='1';list.addEventListener('click',e=>{const b=e.target.closest('[data-one-local]');if(!b)return;e.preventDefault();e.stopImmediatePropagation();openLocal(b.dataset.oneLocal)},true)}
}

function currentRecord(id=''){return window.State?.records?.find?.(r=>String(r.id)===String(id))||null}
async function openTramoFromLast(){
  const id=window.State?.lastShotId||'';if(!id)return toast('Primero toma una foto representativa del tramo.');pendingTramoEvidenceId=id;
  try{await window.ONE_V661_IDLE_LOADER?.ensure?.('territory')}catch(_){}
  try{window.UI?.setView?.('Places')}catch(_){}
  setTimeout(()=>{patchTramoStart();const card=$('v6413CorridorCard');card?.scrollIntoView?.({behavior:'smooth',block:'start'});toast('🪧 Completa el tramo. Al iniciar, esta foto quedará como primera muestra.',3600)},120);
}
function ensureQuickTramo(){
  const bar=$('quickCaptureBar');if(!bar||$('one6618QuickTramo'))return;const b=document.createElement('button');b.id='one6618QuickTramo';b.type='button';b.textContent='🪧 Tramo';b.title='Usar esta foto como muestra de un tramo repetitivo';const close=$('quickCaptureClose');bar.insertBefore(b,close||null);b.onclick=openTramoFromLast;
}
function patchTramoStart(){
  const C=window.PropagandaCorridor;if(!C?.start||C.__one6618PendingSample)return false;C.__one6618PendingSample=true;const old=C.start.bind(C);C.start=async(...args)=>{const out=await old(...args);if(C.current?.()&&pendingTramoEvidenceId){const r=currentRecord(pendingTramoEvidenceId);if(r){try{C.addEvidence?.(r,{source:'quick-tramo'});toast('📸 Primera foto vinculada al tramo')}catch(_){}}pendingTramoEvidenceId=''}return out};return true;
}
...
function patchSync(){
  ...
  if(!$('cloudSyncOneLatest')){const allBtn=$('cloudSyncAll');if(allBtn){const b=document.createElement('button');b.id='cloudSyncOneLatest';b.type='button';b.textContent='☁ Probar última evidencia';allBtn.insertAdjacentElement('beforebegin',b);b.onclick=async()=>{const p=$('cloudSyncProgress'),key=$('cloudSyncKey')?.value?.trim()||'';Cloud.saveConfig?.({apiBase:location.origin,syncKey:key});const r=(window.State?.records||[]).find(x=>x?.image);if(!r){if(p)p.textContent='No hay una evidencia local para probar.';return}if(!key){if(p)p.textContent='Guarda la clave primero.';return}b.disabled=true;const old=b.textContent;b.textContent='Sincronizando 1…';try{await Cloud.syncRecord(r,true);if(p)p.textContent=`✓ 1 evidencia sincronizada · ${r.photoCode||r.id}`;toast('✓ Prueba de una evidencia sincronizada',2600)}catch(err){if(p)p.textContent=`✗ ${err.message||err}`;toast(`No se pudo sincronizar · ${err.message||err}`,4000)}finally{b.disabled=false;b.textContent=old}}}}
  return true;
}

function migrationArrival(){
  const params=new URLSearchParams(location.search);if(params.get('from')!=='one-shop'||$('one6618Arrival'))return;
  const d=document.createElement('div');d.id='one6618Arrival';d.style.cssText='position:fixed;left:12px;right:12px;bottom:calc(12px + env(safe-area-inset-bottom));z-index:2147483000;background:#071b37;color:#fff;border:1px solid #315781;border-radius:18px;padding:13px;box-shadow:0 18px 50px rgba(0,0,0,.4);display:grid;gap:9px';d.innerHTML='<b>✓ Ya estás en ONE SHOT 2</b><span style="font-size:12px;color:#c8d6e8">La app antigua y esta app no comparten almacenamiento local. Si llegaste mediante el asistente de migración, tus evidencias antiguas se respaldaron primero. Configura el acceso de sincronización una sola vez en este dispositivo.</span><div style="display:flex;gap:8px"><button id="one6618ArrivalSync" style="flex:1;padding:10px;border:0;border-radius:12px;font-weight:900">☁ Configurar sincronización</button><button id="one6618ArrivalClose" style="padding:10px;border:0;border-radius:12px">Cerrar</button></div>';document.body.appendChild(d);$('one6618ArrivalSync').onclick=()=>window.OneShotCloud?.open?.('Configura la clave de este dispositivo y usa “Probar escritura”.');$('one6618ArrivalClose').onclick=()=>d.remove();
}
